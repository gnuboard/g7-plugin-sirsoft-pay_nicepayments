...
- 상품의 기본 통화와 다른 통화로 결제할 때(예: 엔화 상품을 원화로 결제) 결제창 청구 금액과 서명·승인 확인 금액의 통화 기준이 어긋나 결제가 막히던 문제를 수정했습니다. 결제 서명·승인·결제창 닫힘 확인이 고객이 선택한 결제 통화 기준으로 일관되게 검증됩니다.
...
- 마이페이지 주문 영수증의 과세금액 표시가 통화 설정을 따르도록 정리했습니다 — 기본 통화가 달러·엔 등일 때 과세금액이 원화 기호로 고정 표시되던 부분을 주문 통화 기준 표기로 바꿨습니다.
